...
| 12 | `SaveManager` | `scripts/autoloads/SaveManager.gd` | Single-slot JSON save/load of all persistent game state |
...
- `SaveManager` has no `_ready()`-time dependencies; it resolves `PlayerData`, `EconomyManager`, `RankManager`, `ContractManager`, `TradingPostManager`, `CaravanMasterManager`, `TraderManager` on demand when `save_game()`/`load_game()` is called.
...
`MainMenu.tscn` has a `ContinueBtn` (hidden unless a save exists) alongside `StartBtn`. `TopBar.tscn` has a `SaveSlot/SaveButton` for manual saves.
...
scripts/
  ...
  Player.gd         — (legacy)
  MainMenu.gd       — title screen; wires Continue (when a save exists), Start, Quit
```

Note: `autoloads/SaveManager.gd` is listed in the autoload table above; it lives alongside the other autoload scripts.
...
`SaveManager` (`scripts/autoloads/SaveManager.gd`) persists the whole run to a single JSON file: `user://saves/savegame.json`, written with `JSON.stringify(data, "\t")`.

**Triggers:**
- Autosave at the end of every `WorldMap._on_day_tick()` (both the early-return arrival branch and the normal end-of-tick branch).
- Manual save via the `SaveButton` in `TopBar.tscn`, which emits `TopBar.save_requested` → `WorldMap._on_save_requested()`.
- The save is deleted when the player hits the debt-duration game-over condition (`PlayerData._apply_debt_duration_penalties()`, `debt_days >= 60`), so "Continue" never reloads directly into an unwinnable loss state. Returning to the main menu via `GameOver.gd` does **not** touch the save file otherwise — "New Game" simply leaves the old save on disk until the next autosave overwrites it.

**What's saved:** `PlayerData` (gold, debt, inventory, caravan, faction reputation, day, finance buckets), `EconomyManager.towns` (minus `position`, which is re-derived from scene anchors via `WorldMap._sync_town_positions_from_anchors()` on every load), `RankManager.current_rank_index`, `ContractManager.contracts` + its id counter, `TradingPostManager.posts`, `CaravanMasterManager.masters` (serialized `CaravanMaster` Resources) + `.routes`, `TraderManager.traders`, and WorldMap travel/speed/victory state (`is_traveling`, `travel_destination`, `travel_days_remaining`, `travel_total_days`, `travel_elapsed_seconds`, `game_speed`, `victory_summary_shown`).

**What's excluded (and why):** town `position` (Vector2, re-derived from anchors); `CaravanMasterManager.town_candidates` (unowned hire offers, simply regenerated by the existing `call_deferred("refresh_all_candidates")` in `_ready()`); `master_templates` and all CSV-loaded balance data (static, reloaded from `data/balance/*.csv` every `_ready()`); `FactionManager.npcs` (static, rebuilt in `_init_npcs()`); `TravelRiskManager` (fully stateless).

**Load flow:** `MainMenu.gd` sets `SaveManager.pending_load = true` before changing to `WorldMap.tscn` when Continue is pressed. `WorldMap._ready()` checks that flag, calls `SaveManager.load_game()` (which restores every autoload above) before building the scene, then — after town buttons, roads, and the day timer are set up — calls `_apply_loaded_world_state()` to restore travel/speed/victory state via `SaveManager.consume_world_state()`. Mid-travel resume reuses the existing `_update_player_travel_position()` lerp helper and `_set_speed()` rather than duplicating travel math.
